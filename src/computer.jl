...
function run_program_async(_data::Array{Int64, 1}, channel_in::Channel{<:Integer}, channel_out::Channel{<:Integer}; memory_size::Int64 = 5000)
...
function run_program(_data::Array{Int64, 1}; input::Array{Int64, 1}=[1])
    channel_in = Channel{Int64}(Inf)
    channel_out = Channel{Int64}(Inf)
    output = Int64[]

    for i in input
        put!(channel_in, i)
    end
    data = run_program_async(_data, channel_in, channel_out)
